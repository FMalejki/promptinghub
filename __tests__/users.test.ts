...
describe("profiles", () => {
  it("getProfile returns email, name, image", async () => {
    await createUser(db, "a@b.com", "secret123", "Ada");
    expect(await getProfile(db, "a@b.com")).toEqual({ email: "a@b.com", name: "Ada", image: null });
  });
  ...
  it("updateProfile changes name and image", async () => {
    ...
    expect(updated).toEqual({ email: "a@b.com", name: "New Name", image: "http://img/x.png" });
  });
  ...
  it("updateProfile applies a partial update", async () => {
    ...
    expect(await getProfile(db, "a@b.com")).toEqual({ email: "a@b.com", name: "Keep", image: "http://img/y.png" });
  });
});
