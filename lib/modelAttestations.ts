...
export type Vote = "works" | "broken";
export type AttestationRow = { modelId: string; email: string; vote: Vote };
export type ModelSummary = { modelId: string; works: number; broken: number; youVoted: Vote | null };

export function isValidVote(v: unknown): v is Vote {
  return v === "works" || v === "broken";
}
...
export function aggregateAttestations(rows: AttestationRow[], viewerEmail?: string | null): ModelSummary[] {
  ...
  for (const r of rows) {
    ...
    if (!s) {
      s = { modelId: r.modelId, works: 0, broken: 0, youVoted: null };
      map.set(r.modelId, s);
    }
  ...
  return [...map.values()].sort(
    (a, b) => b.works + b.broken - (a.works + a.broken) || a.modelId.localeCompare(b.modelId),
  );
}
