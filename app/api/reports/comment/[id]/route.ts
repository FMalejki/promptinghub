...
import { resolveCommentReport } from "@/lib/commentReports";

// Resolve/dismiss a comment report — verified accounts only.
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const ok = await resolveCommentReport(db, params.id, status);
