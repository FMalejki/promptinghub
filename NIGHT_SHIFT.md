...
- WAVE 6 QUEUE (autonomous, unblocked): (u) trending creators leaderboard /creators (rank by followers+stars); (v) cmd+k command palette quick search; (w) prompt comparison page (diff any two prompts, reuses lib/diff); (x) editable profile bio + links (settings → shown on /u/[handle]). NEXT: trending creators leaderboard.
