...
- WAVE 3 QUEUE (autonomous, unblocked): (i) prompt report/flag (abuse) → moderation queue; (j) "copy as API call" snippet on detail (uses existing API keys + /api/v1); (k) related-by-tag on detail (tag overlap, complements category-related); (l) OG dynamic image route for prompts. NEXT: related-by-tag (small, leverages #39) then report/flag.
