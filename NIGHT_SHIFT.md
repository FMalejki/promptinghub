...
- **21 PRs this session (#14–#34). ENTIRE QUEUE DONE.** NEXT WAVE (propose + build): (a) version restore/revert (completes #33); (b) "forked from" lineage + fork count; (c) RSS/Atom feed of trending (distribution); (d) free-form tags alongside categories; (e) version diff (v1↔v2); (f) account deletion (settings stub is fake).
