...
- NEXT COOL FEATURES QUEUE (keep building per "WSZYSTKO"): (a) ingested-drafts curation UI (review pending → publish, completes X loop); (b) Trending/leaderboard page (by copyCount+stars); (c) comments/discussion on prompts; (d) prompt versioning/changelog; (e) export collection as bundle/manifest; (f) API keys for programmatic fetch; (g) full E2E Preview click-through of Collections/pricing/image badges (needs login).
