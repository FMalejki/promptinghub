...
- PR #79 merged: **browse sort by most viewed** — listPrompts 'viewed' sort (viewCount desc, ties by recency, missing→0; viewCount added to aggregate), /api/prompts accepts sort=viewed, 'Most viewed' toggle on /browse. +3 tests. 357 tests. Live (/api/prompts?sort=viewed returns prompts).
- PR #80 merged: **collection RSS feed** — lib/rss.ts toRssPrompts (drops private, keeps handle/slug/createdAt), /collections/[id]/feed.xml (own channel metadata, 404 unknown, DB-safe) + visible RSS link on collection page. +1 test. 358 tests. Live (unknown collection→404; happy path not exercisable live — no public/non-empty collections in seed data yet, route composition unit-tested). 67 PRs this session (#14–#80).
- WAVE 11 PROGRESS: ✅ #79 sort-by-views, ✅ #80 collection RSS. Remaining: (qq) length badge on cards; (uu) report-comment; (vv) last-updated relative time.
- PR #81 merged: **prompt 'last updated' relative time** — lib/relativeTime.ts (pure N-units-ago, injectable now, just-now/yesterday/weeks→years, future-clamp, invalid→''), updatePrompt stamps updatedAt, getPromptDetail + namespaced mapper expose updatedAt (default null, strict toEqual updated), detail byline shows '· updated <relative>'. +6 tests. 364 tests. Live (/prompt 200).
- PR #82 merged: **report a comment** — lib/commentReports.ts (reportComment deduped/empty-reason-default/not_found, listOpenCommentReports w/ body, resolveCommentReport), POST /api/comments/[id]/report, comment reports in /api/reports + moderation 'Comment reports' section + /api/reports/comment/[id] resolve, 'report' link on comments (non-author). +5 tests. 369 tests. Live (report POST 401 unauth, /api/reports 401, resolve 401). 69 PRs this session (#14–#82).
- ✅ **WAVE 11 COMPLETE** (#79 sort-by-views, #80 collection RSS, #81 last-updated time, #82 report-comment). (qq) length badge on cards still deferred (needs length threaded into listPrompts/PromptCard).
- WAVE 12 QUEUE (autonomous, unblocked): (qq) prompt length badge on cards (thread word/token estimate into listPrompts + PromptCard); (xx) notification "mark one read" + click-through; (yy) prompt "duplicate/clone to my account" one-click; (zz) creator "joined <date>" + total views stat; (a2) browse empty-state "suggest a prompt" CTA.
- PR #83 merged: **prompt length badge on cards** (closes deferred qq) — lib/promptLength.ts lengthLabel (Short ≤150 / Medium ≤600 / Long; null for 0/missing/neg), listPrompts returns optional tokens (promptToText+estimateTokens over body/files), PromptCard shows Short/Medium/Long badge w/ ~tokens tooltip. +4 tests. 373 tests. Live (/api/prompts returns tokens: 74/1211/1047).
- PR #84 merged: **notifications mark-one-read + click-through** — lib/notifications.ts markRead (recipient-scoped, false on bad/not-yours), POST /api/notifications/[id], page no longer auto-marks-all on view: each item marks itself read on click+links through, explicit 'Mark all read' button. +1 test. 374 tests. Live (mark-one POST 401 unauth). 71 PRs this session (#14–#84).
- WAVE 12 PROGRESS: ✅ #83 length badge, ✅ #84 notif mark-one. Remaining: (yy) clone-to-account; (zz) creator joined-date + total views; (a2) browse empty-state CTA.
- PR #85 merged: **creator joined-date + total views** — lib/users.ts creatorStats (joinedAt + summed public views/copies, excludes private, missing→0, unknown→zeros), /api/u/[handle] returns stats, profile shows 'views' stat + 'Joined <month year>'. +4 tests. 378 tests. Live (/api/u/bob stats joinedAt+totalViews:1).
- PR #86 merged: **browse empty-state CTA** — lib/browseFilters.ts hasActiveFilters (q-trim/category/tag/imageOnly), empty state branches: active filters→'Clear filters' reset button, empty catalog→'Create a prompt' CTA → /new. +4 tests. 382 tests. Live (/browse 200). 73 PRs this session (#14–#86).
- NOTE: (yy) clone-to-account skipped — the existing **fork** feature already clones a prompt into your account; a separate "duplicate" would be a near-duplicate.
- ✅ **WAVE 12 COMPLETE** (#83 length badge, #84 notif mark-one, #85 creator stats, #86 browse CTA; yy=fork pre-existing).
- WAVE 13 QUEUE (autonomous, unblocked): (b2) prompt "open in ChatGPT/Claude" quick-action links; (c2) dashboard top-referrers/empty-state polish; (d2) collection cover image; (e2) prompt JSON-LD structured data for SEO; (f2) "copied!" toast consistency pass.
