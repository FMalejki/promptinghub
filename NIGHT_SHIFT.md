...
- WAVE 5 QUEUE (autonomous, unblocked): (q) /c/[category] landing pages + categories index (mirrors tag pages, SEO); (r) in-app notifications (follow/fork/comment) with unread badge; (s) copy-events time series → sparkline on dashboard (new copyEvents collection, real over-time analytics); (t) social share buttons on detail (X intent + copy link). NEXT: /c/[category] landing pages.
